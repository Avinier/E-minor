...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === "POST") {
    try {
      ...
      const output = await model.predict({ prompt: `A illustration of the lyrics '${req.body.lyrics}'. ${req.body.song},triadic color scheme, 8k resolution, highly detailed, epic composition by greg tocchini, virgil finlay. Official art`, 
